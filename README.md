...
- **途中経過のテキストも表示**: Claude がツールを呼ぶ前に書いた説明文を `PreToolUse` のたびに transcript から拾い、最終回答（`Stop` イベント）を待たずに随時ライブアクティビティへ反映する。さらに作業中/圧縮中のセッションは transcript ファイルを **1 秒間隔でも監視**しており、ツールを呼ばず文章を書いているだけの間もフックを待たずに反映する（ファイルサイズ変化のみ毎秒チェックし、変化がなければ再パースしないので軽い）。
