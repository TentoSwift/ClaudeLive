...
enum FrameAnimation {
    ...
    /// コンパクト用 8 スロット方式のフォント（ClaudeLoop00〜07）。
    /// スロット s = 4·par + k（par=秒の偶奇, k=0..3 で 0.25 秒刻みのオフセット）。
    /// 継ぎ目なく厳密に 2 秒周期でループする（10 桁 mod 5 方式は周期の端で
    /// コマが重複し休符が生じたため、偶奇だけを使う方式に作り直した）
    static let loopFontNames = (0..<8).map { String(format: "ClaudeLoop%02d-Regular", $0) }
    ...
    static func registerFont() -> Bool {
        ...
        for name in loopFontNames { register(name, ext: "ttf") }
...
struct CompactSpinnerView: View {
    ...
    /// そこで一切の合成を使わない「時間展開 8 レイヤー」方式で、
    /// ロック画面と同じ 8 コマのモーフィングを厳密に 2 秒周期でループする:
    ///
    ///  - コマ画像は黒背景付き（DI の背景も純黒なので見えない）にすることで、
    ///    最前面（z = スロット番号）のアクティブレイヤーが下を完全に覆う。
    ///    これによりマスク不要で任意のコマ（回転含む）がそのまま使える
    ///  - スロット s = 4·par + k（par = 秒の偶奇、k = 0..3 で 0.25 秒刻みの
    ///    オフセット）。各レイヤー専用のフォントは、担当の偶奇のときだけ
    ///    黒背景付きコマを、逆の偶奇では透明グリフを表示する
    ///  - 10 桁 mod 5 方式（旧版）は 5 秒周期の端で同じコマが重複し休符が
    ///    生まれてしまったため、偶奇だけを使う方式に作り直した。継ぎ目なく
    ///    厳密に 2 秒周期でループする
    var body: some View {
        let base = Date(timeIntervalSinceReferenceDate: 0)
        ZStack {
            ForEach(0..<8, id: \.self) { s in
                Text(base.addingTimeInterval(TimeInterval(s % 4) * 0.25), style: .timer)
                    .font(.custom(FrameAnimation.loopFontNames[s], size: size))
                    .lineLimit(1)
                    .multilineTextAlignment(.trailing)
                    .truncationMode(.head)
                    .dynamicTypeSize(.large)
